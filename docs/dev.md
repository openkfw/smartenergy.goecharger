...
After few minutes, Home Assistant should be running and script should be in the watch mode. Whenever you change a file in the `custom_components/smart_energy_goecharger` folder, it will restart the Home Assistant within a few seconds. Thus, you have a quick development feedback.
...
```
black custom_components/smart_energy_goecharger
```
...
# code coverage
pytest --durations=10 --cov-report term-missing --cov=custom_components.smart_energy_goecharger tests
```
