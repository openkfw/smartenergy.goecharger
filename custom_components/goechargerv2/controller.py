...
from .const import DOMAIN, COMMON_STATE

_LOGGER = logging.getLogger(__name__)


async def fetch_status(hass, charger_name):
    """Fetch go-eCharger car status via API."""

    api = hass.data[DOMAIN][COMMON_STATE]["apis"][charger_name]
...
class ChargerController:
    ...
    def _is_charging_allowed(self, charger_name):
    ...
    async def start_charging(self, call):
        ...
        api = self._hass.data[DOMAIN][COMMON_STATE]["apis"][charger_name]
    ...
    async def stop_charging(self, call):
        """Get name and assigned power from the service call and call the API accordingly.
        In case charging is not allowed, log an error and early escape."""
    ...
    async def change_charging_power(self, call):
    ...
    async def set_authentication(self, call):
